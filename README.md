...
| **Gemini / DeepSeek / Kimi 等** | ❌ 暂不支持 MCP | ⭐ 手动模式可用（或配合 parse_fit.py） |

> **无 MCP 时**：手动粘贴训练数据或使用 `parse_fit.py` 解析 FIT 文件，AI 仍可完整分析和生成课表。

### 模型能力档位划分

| 能力档位 | 要求与特点 | 推荐用途 |
|---|---|---|
| **旗舰推理档** | 支持长文本结构化输出（≥ 32k tokens）与深度逻辑推理 | 生成 15 周全程课表、赛前深度分析 |
| **均衡档** | 具备良好的表格渲染能力与较快响应速度 | 周例行更新、日常单次/周复盘 |
| **轻量档** | 基础对话能力 | 仅限快速查询，不建议生成全程课表 |
...
生成完整课表前，请确认当前模型属于**旗舰推理档**（具备长文本结构化表格输出能力）。
